...
//		Logger.i(LOGTAG, "id:"+id);
	...
	public List<Restaurant> selectList(long campusId,int type){
		...
		String sql = "select * from "+DBOpenHelper.RESTAURANT_TABLE_NAME +" where campusid=?" +addSql;
		Logger.i(LOGTAG, sql);
		Cursor cursor=db.rawQuery(sql, new String[]{String.valueOf(campusId)});
	...
	private Restaurant select(int id){
		db=dbOpenHelper.getWritableDatabase();
		String sql = "select * from "+DBOpenHelper.RESTAURANT_TABLE_NAME +" where _id = ?";
...
//			restaurant.setType(cursor.getInt(4));
			...
			restaurant.setDefaultRank(cursor.getInt(11));
...
//			restaurant.setType(cursor.getInt(4));
	...
	private boolean insert(Restaurant restaurant){
		db=dbOpenHelper.getWritableDatabase();
		Logger.i(LOGTAG, "id:"+restaurant.getId()+" --  campusId:"+restaurant.getCampusId());
...
//		Logger.i(LOGTAG, "isshitang:"+restaurant.getIsShiTang());
		...
		String sqlInsert=
    		"insert into "+DBOpenHelper.RESTAURANT_TABLE_NAME +
			"(_id,name,location,desc,issale,isrest,isshitang,phone,score,imgurl,menuurl,defaultrank,campusid) "+
			"values(?,?,?,?,?,?,?,?,?,?,?,?,?)";
    	
		String phoneNum=Tools.phoneNum(restaurant.getPhoneList());
		
		try{
	    	db.execSQL(sqlInsert, new Object[]{restaurant.getId(),restaurant.getName(),
	    			restaurant.getLocation(),restaurant.getDesc(),restaurant.getIsSale(),restaurant.getIsRest(),restaurant.getIsShiTang(),
	    			phoneNum,restaurant.getScore(),restaurant.getImgUrl(),restaurant.getMenuUrl(),
	    			restaurant.getDefaultRank(),restaurant.getCampusId()});
	...
	private boolean update(Restaurant restaurant){
		db=dbOpenHelper.getWritableDatabase();
		String sqlUpdate="update "+DBOpenHelper.RESTAURANT_TABLE_NAME +" set " +
				"name = ?,location=?,desc=?,issale=?,isrest=?,isshitang=?,phone=?,score=?,imgurl=?,menuurl=?,defaultrank=?,campusid=? " +
				"where _id = ?";
		String phoneNum=Tools.phoneNum(restaurant.getPhoneList());
		try{
			db.execSQL(sqlUpdate, new Object[]{restaurant.getName(),
	    			restaurant.getLocation(),restaurant.getDesc(),restaurant.getIsSale(),restaurant.getIsRest(),restaurant.getIsShiTang(),
	    			phoneNum,restaurant.getScore(),restaurant.getImgUrl(),restaurant.getMenuUrl(),
	    			restaurant.getDefaultRank(),restaurant.getId(),restaurant.getCampusId()});
		}catch (SQLException e) {
