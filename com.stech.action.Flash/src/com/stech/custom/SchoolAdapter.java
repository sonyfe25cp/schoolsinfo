...
//			Logger.i(LOGTAG, ""+(tmp==null?true:false));
			group.add(tmp);
//			Logger.i(LOGTAG, "tmp.getCampusList:"+tmp.getCampusList()==null?true+"":tmp.getCampusList().size()+"");
			child.add(tmp.getCampusList());
		}
//		Logger.i(LOGTAG, "child.size: "+child.size());
//		Logger.i(LOGTAG, "group.size: "+group.size());
		mIcon=BitmapFactory.decodeResource(activity.getResources(), R.drawable.icon);
	}
	...
	public View getView(int groupPosition, int childPosition,
	   ...
	   if (convertView == null) {
			...
			holder.textView = (TextView) convertView.findViewById(R.id.campus_name);
			holder.icon = (ImageView) convertView.findViewById(R.id.campus_icon);
			convertView.setTag(holder);
	   }else {
		   	holder = (ViewHolder) convertView.getTag();
	   }
	   String name = child.get(groupPosition).get(childPosition).getCampus_name();
